...
#define SW_TEMPLATE "FROM/A,OLD/S"
...
void o5_SimpleWrite(int argc, char **argv)
{
    ...
    len=jsize(inbuffer);
    if (sw_argarray.as_old)
	sendfile_old(inbuffer,len);
    else
...
void o5_SimpleRead(int argc, char **argv)
{
    ...
    if (sr_argarray.as_old)
	receivefile_old(&inbuffer,&len);
    else
...
*/
    slowsendnew(newbuffer,0x200,150,"BOOTER");
...
*/
    slowsendnew(newbuffer,newlen,600,"DATA");
